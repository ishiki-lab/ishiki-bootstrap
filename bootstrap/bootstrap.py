...
def start():
    print("Bootstrap from USB drive: Starting")
    ...
    if mount_usb():
        ...
        if ssid:
            if not already_has_creds(ssid, psk):
                print("Ishiki Bootstrap from USB drive: Adding SSID=%s and PASSWORD=%s to wpa_supplicant.conf" % (ssid, psk))
                add_wifi_creds(ssid, psk)
            else:
                print("Ishiki Bootstrap from USB drive: Wifi already configured for %s" % ssid)
        else:
            print("Ishiki Bootstrap from USB drive: No ssid given")
        ...
        if tunnel_host and docker_tunnel_port and admin_tunnel_port and tunnel_user:
            print("Ishiki Bootstrap from USB drive: Docker tunnel configured to %s on port %s" % (tunnel_host, docker_tunnel_port))
            configure_ssh_tunnel(tunnel_host, docker_tunnel_port, tunnel_user, "2375", "docker_tunnel")
            print("Ishiki Bootstrap from USB drive: SSH tunnel configured to %s on port %s" % (tunnel_host, admin_tunnel_port))
            configure_ssh_tunnel(tunnel_host, admin_tunnel_port, tunnel_user, "22", "admin_tunnel")
    else:
        print("Ishiki Bootstrap from USB drive: Failed to find the ishiki USB")
...
def set_time_zone(time_zone):
    ...
    if os.path.exists(dst):
        if os.path.islink(dst):
            ...
            if target == src:
                print("Ishiki Bootstrap from USB drive: Time zone already set to %s" % time_zone)
                return
            else:
                os.remove(dst)
        else:
            os.remove(dst)
    print("Ishiki Bootstrap from USB drive: Setting time zone to %s" % time_zone)
    os.symlink(src, dst)
...
def ensure_content_matches(file_path, content, mode, uid, gid):
    ...
    if os.path.exists(file_path):
        ...
        if existing_content == content:
            print("Ishiki Bootstrap from USB drive: File alread exists %s" % file_path)
            return
        else:
            os.remove(file_path)

    print("Ishiki Bootstrap from USB drive: Updating file %s" % file_path)
...
def set_hostname(new_hostname):
    ...
    if new_hostname != current_hostname:
        print("Ishiki Bootstrap from USB drive: Setting host name to %s" % new_hostname)
        ...
        print("Ishiki Bootstrap from USB drive: Host name already set to %s" % new_hostname)
...
def set_ip_address(interface, address=None, netmask=None, router=None):
    ...
    if address:
        print("Ishiki Bootstrap from USB drive: Setting a static ip address of %s for %s" % (address, interface))
    else:
        print("Ishiki Bootstrap from USB drive: Setting %s to use DHCP" % interface)
